...
public class UmbrellaDetector_v3 : MonoBehaviour {
    ...
    void Update()
    {
        ...
        /*
        umbrellaCenter = GetLed2DPosition(BucketSize); // en pixels

        // la détection n'a pas fonctionné (OU le point trouvé n'est pas assez lumineux)
        if (umbrellaCenter.Equals(Vector2.zero))
        {
            wasDetectedOnPreviousFrame = false;
            return;
        }

        if (wasDetectedOnPreviousFrame)
        {
            // ACTIVATION DE L'INTERACTION
            print("Looking for interaction.");
            enableAction = true;
            wasDetectedOnPreviousFrame = false; 
        }
        ...
        testGameObject.transform.position = new Vector3(umbrellaCenter.x * Screen.width, umbrellaCenter.y * Screen.height, -1.0f);
    ...
    #region ANCIEN CODE : avec détection de la luminosité
    /*
